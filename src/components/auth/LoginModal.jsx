          ...
          <div className="space-y-4 pt-2">
            ...
            {!showOtp ? (
              <form onSubmit={handleSendOtp} className="space-y-3.5">
                <div className="relative border border-zinc-200 rounded-xl overflow-hidden flex items-center bg-zinc-50 focus-within:border-black transition-colors">
                  <span className="px-3 text-xs font-bold text-black border-r border-zinc-200 py-3.5 bg-zinc-100 shrink-0">
                    +91
                  </span>
                  <input
                    type="tel"
                    ...
                    className="w-full bg-transparent text-xs py-3 px-3 focus:outline-none font-mono font-bold placeholder:font-normal text-black"
                  />
                </div>

                <label className="flex items-center gap-2 text-[10px] text-zinc-600 font-mono select-none cursor-pointer">
                  <input
                    type="checkbox"
                    checked={notifyMe}
                ...
                <button
                  ...
                  className="w-full py-3.5 bg-zinc-800 hover:bg-black text-white font-extrabold text-xs uppercase tracking-widest transition-all rounded-xl press disabled:opacity-40 shadow-md"
                >
                  Submit
                </button>
              </form>
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-3.5 animate-fade-in">
                {otpNotice && (
                  <div className="bg-emerald-50 border border-emerald-300 text-emerald-950 p-2.5 rounded-xl text-[10px] font-mono font-bold leading-relaxed">
                    {otpNotice}
                  </div>
                )}
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-50 border border-zinc-300 text-center text-lg tracking-[0.4em] py-2.5 focus:outline-none focus:border-black font-mono font-bold rounded-xl"
                  />
                </div>
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-3 bg-zinc-50 border border-zinc-200 hover:border-black text-black font-bold text-[11px] uppercase tracking-wider flex items-center justify-center gap-2 rounded-xl transition-all"
            >
